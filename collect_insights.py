...
## ПРАВИЛА
1. Извлекай практический опыт из реальных кейсов.
2. Конкретные валюты, города, имена, бренды, названия каналов — НЕ включай.
3. Цифры давай в ОТНОСИТЕЛЬНЫХ показателях: доли %, соотношения, пропорции, сроки в месяцах, коэффициенты. Например: "ФОТ составляет 40-50% от выручки", "окупаемость 10-14 месяцев", "фудкост 28-35%".
4. Если в видео звучат абсолютные цифры — пересчитай в относительные (доля от выручки, от OPEX, от инвестиций).
5. Фокус: риски, ошибки, организация, юнит-экономика, от чего зависит успех.
6. НЕ мотивация, НЕ реклама, НЕ пересказ видео.
7. Русский язык. Перефразируй, не копируй.
...
- (5–8 пунктов с конкретными пропорциями в %)
- Пример формата: "ФОТ: 40–50% от выручки", "Аренда: 15–25% от выручки", "Фудкост: 28–35%", "Маркетинг: 3–5% от оборота"
- Средний чек относительно конкурентов (выше/ниже/на уровне рынка)
- Загрузка для точки безубыточности (% от максимальной мощности)
- Типичная маржинальность по форматам (эконом / стандарт / премиум)

## Финансовые риски и ловушки
- (3–5 пунктов: что убивает кеш-флоу)
